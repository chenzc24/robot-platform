...
def _servo_id_byte(servo_id):
    servo_id = int(servo_id)
    if servo_id < _SERVO_ID_MIN or servo_id > _SERVO_ID_MAX:
        raise ValueError("servo_id must be in 0~254: %s" % servo_id)
    return servo_id
...
class ServoBus:
    ...
    def _angle_speed_content(self, servo_id, angle_deg,
        ...
        content = bytearray()
        content.append(_servo_id_byte(servo_id))
    ...
    def ping(self, servo_id, timeout_ms=40):
        servo_id = _servo_id_byte(servo_id)
        content = bytearray([servo_id])
        try:
            clear_uart_input(self.uart)
            packet = build_servo_packet(SERVO_CMD_PING, content)
            self.uart.write(packet)
            servo_tx_wait(len(packet))
            expected_id = bytes([servo_id])
    ...
    def read_angles(self, servo_ids, timeout_ms=None):
        ...
        """
        ids = [_servo_id_byte(sid) for sid in servo_ids]
        if not ids:
            return []
    ...
    def reset_turns(self, servo_id):
        ...
        content = bytearray([_servo_id_byte(servo_id)])
        clear_uart_input(self.uart)
