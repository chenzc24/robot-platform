...
PROBE=/root/robot-platform/arm/l3_probe.py
CONFIRMATION=EXECUTE_FIXED_J1_STEP
supervisor_pid=""

if [ "${1:-}" != "$CONFIRMATION" ]; then
    echo '{"ok":false,"state":"safe_idle","error_code":"safety_confirmation_required"}'
    exit 2
fi
...
if [ "$#" -ne 1 ] || [ "$(readlink "/proc/$1/exe" 2>/dev/null || true)" != "$SUPERVISOR_EXE" ]; then
    echo '{"ok":false,"state":"fault","error_code":"launcher_supervisor_unverified"}'
    exit 1
fi
supervisor_pid="$1"
...
set -- $owner_pids
if [ "$#" -ne 1 ]; then
    echo '{"ok":false,"state":"fault","error_code":"uart_owner_unverified"}'
    exit 1
fi
owner_pid="$1"
if [ "$(readlink "/proc/$owner_pid/exe" 2>/dev/null || true)" != "$LAUNCHER_EXE" ]; then
    echo '{"ok":false,"state":"fault","error_code":"uart_owner_not_launcher"}'
    exit 1
fi
...
if [ "$#" -gt 0 ]; then
    if [ "$#" -ne 1 ] || [ "$1" != "$owner_pid" ]; then
        echo '{"ok":false,"state":"fault","error_code":"uart_owner_changed"}'
        exit 1
    fi
    if [ "$(readlink "/proc/$owner_pid/exe" 2>/dev/null || true)" != "$LAUNCHER_EXE" ]; then
        echo '{"ok":false,"state":"fault","error_code":"launcher_identity_changed"}'
        exit 1
    fi
    kill -KILL "$owner_pid"
    attempt=0
...
if [ -n "$current_owner" ]; then
    echo '{"ok":false,"state":"fault","error_code":"launcher_did_not_stop"}'
    exit 1
fi
if fuser "$UART_DEVICE" >/dev/null 2>&1; then
    echo '{"ok":false,"state":"fault","error_code":"uart_still_busy"}'
    exit 1
fi

python3 "$PROBE" --device "$UART_DEVICE" --baud 115200 --timeout 15.0 --confirmation "$CONFIRMATION"
