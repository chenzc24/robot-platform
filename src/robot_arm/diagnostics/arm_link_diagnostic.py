"""LAN1 responder with one injected, fixed validation motion callback."""
...
class DiagnosticSession:
    """Convert complete valid ping frames into matching pong frames."""
...
def serve(tcp_create, tcp_start, tcp_read, tcp_write, fixed_step=None):
    """Run the diagnostic server through injected controller TCP APIs."""
    error, socket_id = tcp_create(True, "192.168.5.1", 5200)
    if error:
        raise RuntimeError("tcp_create_failed")
    result = tcp_start(socket_id, 0)
    if api_failed(result):
        raise RuntimeError("tcp_start_failed")
    session = DiagnosticSession(fixed_step)
    while True:
        error, received = tcp_read(socket_id)
        if error:
            raise RuntimeError("tcp_read_failed")
        for response in session.feed(received):
            result = tcp_write(socket_id, response)
            if api_failed(result):
