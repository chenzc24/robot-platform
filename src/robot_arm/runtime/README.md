...
whitelist exposes `MovJ`, `MovL`, `RelJointMovJ`, `RelMovLUser`, `Wait`, `CheckMovJ`, and
`CheckMovL`. It does not expose the TCP/IP API names `GetCurrentCommandID` or
`RobotMode` to a DobotStudio Python project. Motion calls therefore follow the
